...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options['all'] or options['absences']:
            ...
            self.stdout.write(self.style.SUCCESS(f'  Sent {len(notifs)} absence notifications'))

        if options['all'] or options['grades']:
            self.stdout.write('Checking grades...')
            notifs = check_and_send_grade_notifications()
            notifications_sent += len(notifs)
            self.stdout.write(self.style.SUCCESS(f'  Sent {len(notifs)} grade notifications'))
        ...
        if options['all'] or options['payments']:
            ...
            notifications_sent += len(notifs)
            self.stdout.write(self.style.SUCCESS(f'  Sent {len(notifs)} payment notifications'))
        ...
        if options['all'] or options['events']:
            ...
            notifications_sent += len(notifs)
            self.stdout.write(self.style.SUCCESS(f'  Sent {len(notifs)} event notifications'))
