...
import { AGENDA_ROUTE } from "@/config";
...
describe("useAgenda", () => {
	...
	it("should fetch agenda data successfully", async () => {
		const mockAgenda = Array.from({ length: 3 }, () => createFakeAgendaItem());
		...
		expect(result.current.error).toBeNull();
		expect(api.get).toHaveBeenCalledWith("/api/school/agenda/");
	});
	...
	it("should refetch agenda data when refetch is called", async () => {
		const mockAgenda1 = Array.from({ length: 2 }, () => createFakeAgendaItem());
		const mockAgenda2 = Array.from({ length: 5 }, () => createFakeAgendaItem());
