...
import { NotificationProps, FakeNotification } from "@/types/notification";
...
export function useNotification() {
	...
	const generateMockNotifications = useCallback((): NotificationProps[] => {
		return Array.from({ length: 5 }, (_, i) => ({
			...
			notification_type: ["GRADE", "ABSENCE", "EVENT", "GENERAL"][i % 4] as any,
			read: i % 2 === 0,
		}));
	}, []);
	...
	const fetchData = useCallback(async () => {
		...
		try {
			const response = await api.get<NotificationProps[]>(`${ROUTES.NOTIFICATIONS}`);
	...
	const markAsRead = useCallback(async (id: number) => {
		try {
			const url = ROUTES.NOTIFICATIONS_MARK_READ.replace("{id}", id.toString());
			await api.post(url);
