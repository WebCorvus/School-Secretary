...
export default function ResourcesPage() {
	...
	if (resourcesError) 
		return (
			<FullScreenError 
				error={resourcesError || "Erro ao carregar informações de recursos."} 
			/>
		);
	...
	return (
		<div className="space-y-6">
			...
			{Object.values(ResourceType).map((type) => {
				const resourcesByType = resources.filter(resource => resource.resource_type === type);
				...
				return (
					<Card key={type}>
						<CardHeader>
							<CardTitle className="text-xl">{getResourceTypeDisplay(type)}</CardTitle>
						</CardHeader>
						<CardContent>
									...
									>
										<div className="flex justify-between items-start">
											...
											<div className="flex flex-col items-end gap-2">
												...
												>
													{getResourceStatusDisplay(resource.status)}
												</Badge>
												<Badge 
													variant="outline"
												>
