                        ...
                        <ul className="list-disc pl-5">
                            {agendaItems.map(item => (
                                <li key={item.id} className="mb-2">
                                    <span className="font-semibold">{item.title}</span> - {item.date} {item.time}
                                    {item.description && <p className="text-sm text-[var(--light-gray)] ml-4">{item.description}</p>}
                                </li>
                            ))}
                        </ul>
                    ) : (
                        <p className="text-[var(--light-gray)]">Nenhuma atualização recente na agenda.</p>
                    )}
                </div>
            </div>
                        ...
                        <ul className="list-disc pl-5">
                            {events.map(event => (
                                <li key={event.id} className="mb-2">
                                    <span className="font-semibold">{event.title}</span> - {event.start_date} {event.start_time}
                                    {event.location && <span className="ml-2 text-sm text-[var(--light-gray)]">({event.location})</span>}
                                    {event.description && <p className="text-sm text-[var(--light-gray)] ml-4">{event.description}</p>}
                                </li>
                            ))}
                        </ul>
                    ) : (
                        <p className="text-[var(--light-gray)]">Nenhum evento recente.</p>
                    )}
                </div>
            </div>
        </div>
